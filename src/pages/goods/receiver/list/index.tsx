...
const ReceiverPage = (props: any) => {
  ...
  const selectAddress = (address) => {
    ...
    history.push('/goods/checkout', params);
  }
        ...
        {address.map((item: any, index: number) => (
            <div key={index} className={Styles.item}>
              ...
              <div className={Styles.option}>
                <div onClick={() => { selectAddress(item) }} style={{flex:1,display:'flex',alignItems:'center',cursor:'pointer'}}>
                  <img className={Styles.icon} src={item.isDefault ? Assets.select : Assets.noSelect} style={{width:'18px',height:'18px'}}/>
                  <div className={Styles.default} style={{fontSize:'13px'}}>{item.isDefault?'默认':'选择'}</div>
                </div>
                <div className={Styles.itemButton} onClick={() => handleDelete(index)}>
                  <div className={Styles.text} style={{fontSize:'13px'}}>删除</div>
                </div>
                <div className={Styles.itemButton} onClick={() => {history.push('/goods/receiver/modify',{'detail':item})}}>
                  <div className={Styles.text} style={{fontSize:'13px'}}>编辑</div>
                </div>
              </div>
            </div>
          ))}
        <div className={Styles.addItem} onClick={() => {history.push('/goods/receiver/add')}}>
