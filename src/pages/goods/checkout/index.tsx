...
const AppPage = (props: any) => {
  ...
  const { goods, quantity, skuKey } = location.state;
  ...
  const [totalPrice, setTotalPrice] = useState(parseFloat(goods.price * quantity).toFixed(2));
            ...
            if (selectedMethod === 'wxPayNative') {
              if (response.data.code === "NO_AUTH") {
                ...
                setTimeout(() => {
                  ...
                  getPaymentStatus(res.data.orderSn, response.data.paymentSn);
                }, 2000);//延迟2秒钟再执行
              }
    ...
    <div style={{background:appInfo?.theme?.bgColor,padding:'15px',borderRadius:'15px'}}>
      ...
      <div className={styles.pay}>
        ...
        <div className={styles.info}>
          <img src={goods.images[0].path}></img>
          <div className={styles.content}>
            <div className={styles.name}>{goods.fullName}</div>
            <div className={styles.price}>¥ {goods.price}</div>
          </div>
          <div className={styles.num}>x{quantity}</div>
        </div>
        <div style={{ background: appInfo?.theme?.bgColor, width: '100%', height: '8px' }} />
      ...
      <Modal
        ...
        closable={true}
        maskClosable={false}
        onCancel={() => { setVisible(false); requestStatusEnable = false; history.push({ pathname: '/user/order/detail/' + orderSn }); }}
        footer={false}
        width={360}
        style={{top:'35%'}}
