...
const AppPage = (props: any) => {
  ...
  const loadMore = () => {
    ...
    const result = api.user.points.recharges({'pageNumber': paginationProps.current,'pageSize': paginationProps.pageSize});
    result.then((res) => {
      message.destroy();
      if (res.success) {
  ...
  return (
    <>
      <div className='menu-content-title' style={{fontSize:'15px',lineHeight:'40px'}}><p>积分充值记录</p></div>
