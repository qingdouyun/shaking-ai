      ...
      >
        {
          dataSource.goods.length>0?
            dataSource.goods.map((item, index) => {
              return (
                <Col key={index} span={8} className="temp3-list" style={{flex:'0 0 32%',padding:'0',display:'block',marginBottom:'10px',borderRadius: `${dataSource.borderRadius}px`}} onClick={()=>{history.push('/goods/detail/' + item.key);}}>
                  <div className="picture" style={{height:'auto'}}>
                    <img style={{width:'100%',display:'block',borderRadius: `${dataSource.borderRadius}px ${dataSource.borderRadius}px 0 0`}} src={item.image} alt="商品图片"/>
                  </div>
                  <div 
                    style={{padding:'5px',background:'#ffffff',borderRadius: `0 0 ${dataSource.borderRadius}px ${dataSource.borderRadius}px`}}
                  >
                    <p className={styles.title}>{item.name}</p>
                    <p className={styles.sales} style={{marginTop:'5px',fontSize:'12px'}}>已售{item.sales}件</p>
